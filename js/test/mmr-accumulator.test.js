 ...
 * @param {Object} block
 * @returns {{txids: Uint8Array[], root: Uint8Array}}
 */
function parseBlockTestVector(block) {
    return {
 ...
 * @param {number} cpHeight
 * @param {number} height
 * @param {Object} proof
 * @returns {{cpHeight: number, height: number, root: Uint8Array, leaf: Uint8Array, branch: Uint8Array[]}}
 */
async function parseProofTestVector(cpHeight, height, proof) {
    // Parse header and compute leaf hash (sha256d of 80-byte header)
    const headerHex = proof.header;
    const header = new Uint8Array(80);
    for (let i = 0; i < 80; i++) {
        header[i] = parseInt(headerHex.substr(i * 2, 2), 16);
    }

    // Compute sha256d of header
    const hash1 = await crypto.subtle.digest('SHA-256', header);
    const hash2 = await crypto.subtle.digest('SHA-256', hash1);
    const leaf = new Uint8Array(hash2);
...
/**
 * Load block hashes as Uint8Array[].
 * @returns {Uint8Array[]}
 */
function loadBlockHashes() {
 ...
 */
function assertHashEquals(actual, expected, message) {
    assert.ok(
...
/**
 * Compute sha256d(left || right) - same as in mmr-accumulator.js
...
async function sha256d(left, right) {
    ...
    concat.set(left, 0);
    concat.set(right, 32);
    const hash1 = await crypto.subtle.digest('SHA-256', concat);
    const hash2 = await crypto.subtle.digest('SHA-256', hash1);
...
describe('MMRAccumulator', () => {
    ...
    describe('empty accumulator', () => {
        it('has correct initial state', async () => {
            ...
            const root = await MMR.getRoot(acc);
            assertHashEquals(root, new Uint8Array(32));
        });

        it('rejects proof verification', async () => {
            const acc = MMR.empty();

            const result1 = await MMR.verifyProofToPeak(acc, 0n, new Uint8Array(32), []);
            assert.strictEqual(result1, false);

            const result2 = await MMR.verifyProofToRoot(acc, 0n, new Uint8Array(32), []);
            assert.strictEqual(result2, false);
        });
    });

    describe('single leaf', () => {
        it('has correct state and root', async () => {
            const hashes = loadBlockHashes();
            let acc = MMR.empty();
            acc = await MMR.extend(acc, hashes[0]);
        ...
        it('verifies empty proofs', async () => {
            const hashes = loadBlockHashes();
            let acc = MMR.empty();
            acc = await MMR.extend(acc, hashes[0]);

            // Proof to peak: empty (leaf is its own peak)
            const peakResult = await MMR.verifyProofToPeak(acc, 0n, hashes[0], []);
            assert.strictEqual(peakResult, true);

            // Proof to root: empty (single leaf is root)
            const rootResult = await MMR.verifyProofToRoot(acc, 0n, hashes[0], []);
    ...
    describe('root matches merkle', () => {
        for (const block of TEST_BLOCKS) {
            it(`matches merkle root for block with ${block.tx.length} txs`, async () => {
                const tv = parseBlockTestVector(block);

                let acc = MMR.empty();
                for (const txid of tv.txids) {
                    acc = await MMR.extend(acc, txid);
                }
    ...
    describe('root is peak for power of two', () => {
        it('has single peak equal to root for 8 leaves', async () => {
    ...
    describe('two peaks', () => {
        it('has correct peak structure for 10 leaves', async () => {
            ...
            let acc8 = MMR.empty();
            for (let i = 0; i < 8; i++) {
                acc8 = await MMR.extend(acc8, tv.txids[i]);
            }
            const root8 = await MMR.getRoot(acc8);

            // Build from last 2 leaves (independent)
            let acc2 = MMR.empty();
            for (let i = 8; i < 10; i++) {
                acc2 = await MMR.extend(acc2, tv.txids[i]);
            }
            const root2 = await MMR.getRoot(acc2);

            // Build from all 10 leaves
            let acc10 = MMR.empty();
            for (const txid of tv.txids) {
                acc10 = await MMR.extend(acc10, txid);
            }
            const root10 = await MMR.getRoot(acc10);
            ...
            const dup1 = await sha256d(root2, root2);      // height 2
            const dup2 = await sha256d(dup1, dup1);        // height 3
            const manualRoot = await sha256d(root8, dup2); // height 4
    ...
    describe('rewind pattern', () => {
        it('can verify past state by extending', async () => {
            ...
            let acc9 = MMR.empty();
            for (let i = 0; i < 9; i++) {
                acc9 = await MMR.extend(acc9, tv.txids[i]);
            }

            // Build from all 11 leaves
            let acc11 = MMR.empty();
            for (const txid of tv.txids) {
                acc11 = await MMR.extend(acc11, txid);
            }
            ...
            candidate = await MMR.extend(candidate, tv.txids[9]);
            candidate = await MMR.extend(candidate, tv.txids[10]);
    ...
    describe('proof to root', () => {
        ...
        for (const { cpHeight, height, proof } of testCases) {
            it(`verifies proof for cp_height=${cpHeight}, height=${height}`, async () => {
                const hashes = loadBlockHashes();
                const tv = await parseProofTestVector(cpHeight, height, proof);
                const leafCount = BigInt(cpHeight + 1);

                // Build accumulator to this leaf count
                let acc = MMR.empty();
                for (let i = 0n; i < leafCount; i++) {
                    acc = await MMR.extend(acc, hashes[Number(i)]);
                }
                ...
                // Verify proof
                const result = await MMR.verifyProofToRoot(
                    acc,
                    BigInt(height),
                    hashes[height],
                    tv.branch
                ...
                if (tv.branch.length > 0) {
                    ...
                    const badResult = await MMR.verifyProofToRoot(
                        acc,
                        BigInt(height),
                        hashes[height],
                        badBranch
    ...
    describe('proof to peak', () => {
        ...
        for (const { cpHeight, height, proof } of testCases) {
            it(`verifies proof for cp_height=${cpHeight}, height=${height}`, async () => {
                ...
                // Verify proof
                const result = await MMR.verifyProofToPeak(
                    acc,
                    BigInt(height),
                    hashes[height],
                    proofToPeak
                ...
                if (proofToPeak.length > 0) {
                    ...
                    const badResult = await MMR.verifyProofToPeak(
                        acc,
                        BigInt(height),
                        hashes[height],
                        badProof
    ...
    describe('bootstrap from last leaf', () => {
        it('extracts peaks and extends correctly', async () => {
            const hashes = loadBlockHashes();

            // Parse 10_10 test vector: 11 leaves, 3 peaks
            const tv1010 = await parseProofTestVector(10, 10, PROOF_10_10);
            ...
            const tv1111 = await parseProofTestVector(11, 11, PROOF_11_11);
            let acc2 = await MMR.extend(acc, hashes[11]);
            const root2 = await MMR.getRoot(acc2);
            assertHashEquals(root2, tv1111.root);

            // Extend with header 12
            const tv1212 = await parseProofTestVector(12, 12, PROOF_12_12);
            let acc3 = await MMR.extend(acc2, hashes[12]);
            const root3 = await MMR.getRoot(acc3);
    ...
    describe('bootstrap from proof', () => {
        ...
        for (const { cpHeight, height, proof } of testCases) {
            it(`bootstraps correctly for leaf_count=${cpHeight + 1}`, async () => {
                const hashes = loadBlockHashes();
                const tv = await parseProofTestVector(cpHeight, height, proof);
                const leafCount = BigInt(cpHeight + 1);
                ...
                for (let i = 0n; i < leafCount; i++) {
                    refAcc = await MMR.extend(refAcc, hashes[Number(i)]);
                }

                // Bootstrap from last leaf proof
                const bootstrapped = await MMR.bootstrapFromProof(
                    leafCount,
                ...
                const bootRoot = await MMR.getRoot(bootstrapped);
                ...
                if (Number(leafCount) < hashes.length) {
                    const refExtended = await MMR.extend(refAcc, hashes[Number(leafCount)]);
                    const bootExtended = await MMR.extend(bootstrapped, hashes[Number(leafCount)]);
    ...
    describe('bootstrap from proof edge cases', () => {
        it('handles empty accumulator', async () => {
            const result = await MMR.bootstrapFromProof(0n, new Uint8Array(32), []);
            assert.ok(result !== null);
            assert.strictEqual(result.leafCount, 0n);
            assert.ok(MMR.isEmpty(result));

            const root = await MMR.getRoot(result);
            assertHashEquals(root, new Uint8Array(32));
        });

        it('handles single leaf', async () => {
            const hashes = loadBlockHashes();
            const result = await MMR.bootstrapFromProof(1n, hashes[0], []);
        ...
        it('rejects empty proof for leaf_count > 1', async () => {
            const hashes = loadBlockHashes();
            const result = await MMR.bootstrapFromProof(2n, hashes[1], []);
            assert.strictEqual(result, null);
        });

        it('rejects non-empty proof for leaf_count == 0', async () => {
            const hashes = loadBlockHashes();
            const result = await MMR.bootstrapFromProof(0n, new Uint8Array(32), [hashes[0]]);
            assert.strictEqual(result, null);
        });

        it('rejects non-empty proof for leaf_count == 1', async () => {
            const hashes = loadBlockHashes();
            const result = await MMR.bootstrapFromProof(1n, hashes[0], [hashes[1]]);
            assert.strictEqual(result, null);
        });

        it('rejects wrong proof length', async () => {
            const hashes = loadBlockHashes();
            const tv = await parseProofTestVector(10, 10, PROOF_10_10);

            // Too short
            const shortProof = tv.branch.slice(0, -1);
            const resultShort = await MMR.bootstrapFromProof(11n, hashes[10], shortProof);
            assert.strictEqual(resultShort, null);

            // Too long
            const longProof = [...tv.branch, new Uint8Array(32)];
            const resultLong = await MMR.bootstrapFromProof(11n, hashes[10], longProof);
            assert.strictEqual(resultLong, null);
        });

        it('handles power of two (8 leaves)', async () => {
            const hashes = loadBlockHashes();

            // Build reference
            let refAcc = MMR.empty();
            for (let i = 0; i < 8; i++) {
                refAcc = await MMR.extend(refAcc, hashes[i]);
            }
            ...
            const l1_0 = await sha256d(hashes[0], hashes[1]);
            const l1_1 = await sha256d(hashes[2], hashes[3]);
            const l1_2 = await sha256d(hashes[4], hashes[5]);
            const l1_3 = await sha256d(hashes[6], hashes[7]);

            const l2_0 = await sha256d(l1_0, l1_1);
            const l2_1 = await sha256d(l1_2, l1_3);

            // Proof for leaf 7: [h6, L1[2], L2[0]]
            const proof = [hashes[6], l1_2, l2_0];

            const result = await MMR.bootstrapFromProof(8n, hashes[7], proof);
            assert.ok(result !== null);
    ...
    describe('serialization', () => {
        it('round-trips correctly', async () => {
            ...
            let acc = MMR.empty();
            for (let i = 0; i < 11; i++) {
                acc = await MMR.extend(acc, hashes[i]);
            }
    ...
    describe('state creation', () => {
        ...
        it('clones state immutably', () => {
            ...
            // Modify clone
            cloned.peaks[0] = new Uint8Array(32);
